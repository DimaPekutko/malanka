import { Token, TokenTypesTable } from './../Tokens/TokenType';
...
export class Lexer {
    ...
    tokenize(): Token[] {
        ...
        return this.tokens.filter(token => token.type.name !== TokenTypesTable.space.name);
    }
    private replace_spaces_with_tabs(): void {
        let space_cnt = 0
        for(let i = 0; i < this.tokens.length; i++) {
            if(this.tokens[i].type.name === TokenTypesTable.space.name) {
                space_cnt++
            }
            else {
                space_cnt = 0;
            ...
            if(space_cnt === this.TAB_SIZE) {
                let tab_token = 
                    new Token(TokenTypesTable.tab, "\t", this.tokens[i].row, this.tokens[i].col)
    ...
    private parse_token(): void {
        let token_types = Object.entries(TokenTypesTable)
        for(let i = 0; i < token_types.length; i++) {
            ...
            if(match !== null) {
                if(match.index === 0) {
                    ...
                    if(token_types[i][1] === TokenTypesTable.new_line) {
                        this.col = 1
                    }
                    return
                }
